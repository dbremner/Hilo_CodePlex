...
HiloPage::HiloPage() 
{
    ...
    // Use methods to avoid inadvertent circular references when
    // subscribing to events with lambda expressions.
    Loaded += ref new RoutedEventHandler(this, &HiloPage::OnLoaded);
    Unloaded += ref new RoutedEventHandler(this, &HiloPage::OnUnloaded);
    ...
    //            &HiloPage::CoreDispatcher_AcceleratorKeyActivated);
    ...
    //            &HiloPage::CoreWindow_PointerPressed);
...
static DependencyProperty^ _defaultViewModelProperty =
    ...
    TypeName(IObservableMap<String^, Object^>::typeid), TypeName(HiloPage::typeid), nullptr);

/// <summary>
/// Identifies the <see cref="DefaultViewModel"/> dependency property.
/// </summary>
DependencyProperty^ HiloPage::DefaultViewModelProperty::get()
{
...
/// </summary>
IObservableMap<String^, Object^>^ HiloPage::DefaultViewModel::get()
{
...
/// used as a trivial view model.
/// </summary>
void HiloPage::DefaultViewModel::set(IObservableMap<String^, Object^>^ value)
{
...
/// <param name="sender">Instance that triggered the event.</param>
/// <param name="e">Event data describing the conditions that led to the event.</param>
void HiloPage::GoForward(Object^ sender, RoutedEventArgs^ e)
{
    (void) sender;	// Unused parameter
    (void) e;	// Unused parameter
...
void HiloPage::CoreDispatcher_AcceleratorKeyActivated(CoreDispatcher^ sender,
...
/// <param name="sender">Instance that triggered the event.</param>
/// <param name="args">Event data describing the conditions that led to the event.</param>
void HiloPage::CoreWindow_PointerPressed(CoreWindow^ sender, PointerEventArgs^ args)
{
...
/// <seealso cref="InvalidateVisualState"/>
String^ HiloPage::DetermineVisualState(ApplicationViewState viewState)
{
    switch (viewState)
    {
...
{
    if (nullptr == viewModel || HiloPage::IsSuspending) return ;
...
void HiloPage::OnLoaded(Object^ sender, RoutedEventArgs^ e)
{
    this->StartLayoutUpdates(sender, e);
...
void HiloPage::OnUnloaded(Object^ sender, RoutedEventArgs^ e)
{
    this->StopLayoutUpdates(sender, e);
    if (_navigationShortcutsRegistered)
    {
